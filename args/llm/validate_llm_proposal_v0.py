...
def _require_keys(obj: Any, required: set[str], optional: set[str], where: str) -> None:
    ...
    if missing:
        raise GateError(1, "FAIL_CONTRACT_MISSING_FIELD", f"{where}:missing:{','.join(sorted(missing))}")
    unknown = keys - required - optional
    if unknown:
        raise GateError(1, "FAIL_CONTRACT_UNKNOWN_FIELD", f"{where}:unknown:{','.join(sorted(unknown))}")
...
def _read_json_file(path: Path, max_bytes: int, where: str) -> Any:
    try:
        ...
        raise GateError(2, "INFRA_MISSING_INPUT", f"{where}:file_not_found", {"path": str(path)}) from e
    except Exception as e:
        raise GateError(2, "INFRA_IO_ERROR", f"{where}:read_error:{type(e).__name__}", {"path": str(path)}) from e

    if len(data) > max_bytes:
        raise GateError(1, "FAIL_OVERSIZED_INPUT", f"{where}:bytes:{len(data)}", {"path": str(path), "max_bytes": max_bytes})

    try:
        return json.loads(data.decode("utf-8-sig"))
    except Exception as e:
        raise GateError(1, "FAIL_CONTRACT_JSON_INVALID", f"{where}:json_parse_error:{type(e).__name__}", {"path": str(path)}) from e
...
def _sum_and_check_str(s: str, name: str, c: Constraints, total: int) -> int:
    if len(s) > c.max_string_chars:
        raise GateError(1, "FAIL_OVERSIZED_TEXT", f"{name}:len:{len(s)}", {"max_string_chars": c.max_string_chars})
    return total + len(s)


def _validate_job_request(req: Any, repo_root: Path) -> tuple[str, Constraints, list[str], Path, Path]:
    _require_keys(
        req,
        required={"schema", "inputs", "allowed_paths_ref", "prompt_ref", "constraints", "run_id"},
        optional=set(),
        where="job_request",
    )
    ...
    _require_keys(req["inputs"], required={"task", "context_ref"}, optional=set(), where="job_request.inputs")
    ...
    if not isinstance(req["inputs"]["context_ref"], str):
        raise GateError(1, "FAIL_CONTRACT_TYPE_INVALID", "job_request.inputs:context_ref")

    _require_keys(
        req["constraints"],
        required={"max_patch_files", "max_total_chars", "max_string_chars", "forbid_network", "forbid_execute"},
        optional=set(),
    ...
    if not isinstance(c_obj["forbid_network"], bool) or not isinstance(c_obj["forbid_execute"], bool):
    ...
    if not isinstance(req["allowed_paths_ref"], str) or not req["allowed_paths_ref"].strip():
        raise GateError(1, "FAIL_CONTRACT_MISSING_FIELD", "job_request:allowed_paths_ref")
    ...
    allowed_ref_obj = _read_json_file(allowed_paths_ref, max_bytes=200_000, where="allowed_paths_ref")
    _require_keys(allowed_ref_obj, required={"schema", "allowed_paths"}, optional=set(), where="allowed_paths_ref")
    if allowed_ref_obj["schema"] != "allowed_paths_ref_v0":
        raise GateError(1, "FAIL_CONTRACT_SCHEMA_MISMATCH", "allowed_paths_ref:schema")
    if not isinstance(allowed_ref_obj["allowed_paths"], list) or not allowed_ref_obj["allowed_paths"]:
        raise GateError(1, "FAIL_CONTRACT_TYPE_INVALID", "allowed_paths_ref:allowed_paths")

    allowed_paths: list[str] = []
    for i, p in enumerate(allowed_ref_obj["allowed_paths"]):
        if not isinstance(p, str) or not p.strip():
            raise GateError(1, "FAIL_CONTRACT_TYPE_INVALID", f"allowed_paths_ref:allowed_paths[{i}]")
        if not _is_safe_relpath(p):
            raise GateError(1, "FAIL_ALLOWLIST_VIOLATION", "allowed_paths_ref:unsafe_path", {"path": p})
...
def _validate_job_result(res: Any, c: Constraints, allowed_paths: list[str]) -> dict[str, Any]:
    _require_keys(
        res,
        required={"schema", "ok", "proposals", "patch_payload", "diagnostics", "model_id"},
        optional=set(),
        where="job_result",
    )
    ...
    _require_keys(res["diagnostics"], required={"tokens_in", "tokens_out", "notes"}, optional=set(), where="job_result.diagnostics")
    d = res["diagnostics"]
    ...
    total_chars = _sum_and_check_str(str(d["notes"]), "diagnostics.notes", c, total_chars)

    for i, p in enumerate(res["proposals"]):
        _require_keys(p, required={"kind", "text"}, optional=set(), where=f"proposal[{i}]")
        ...
        total_chars = _sum_and_check_str(p["text"], f"proposal[{i}].text", c, total_chars)
    ...
    if patch is None:
        ...
        _require_keys(patch, required={"format", "changes"}, optional=set(), where="patch_payload")
        ...
        if len(patch["changes"]) > c.max_patch_files:
            raise GateError(1, "FAIL_CONTRACT_VALUE_INVALID", "patch_payload:too_many_files", {"max_patch_files": c.max_patch_files})

        patch_info["has_patch"] = True
        patch_info["patch_files"] = len(patch["changes"])

        for i, ch in enumerate(patch["changes"]):
            _require_keys(ch, required={"path", "op"}, optional={"content"}, where=f"patch_change[{i}]")
            if not isinstance(ch["path"], str) or not ch["path"].strip():
                raise GateError(1, "FAIL_CONTRACT_TYPE_INVALID", f"patch_change[{i}]:path")
            if not isinstance(ch["op"], str):
                raise GateError(1, "FAIL_CONTRACT_TYPE_INVALID", f"patch_change[{i}]:op")
            if ch["op"] not in ("write", "delete"):
                raise GateError(1, "FAIL_CONTRACT_VALUE_INVALID", f"patch_change[{i}]:op")

            if not _is_safe_relpath(ch["path"]):
                raise GateError(1, "FAIL_ALLOWLIST_VIOLATION", f"patch_change[{i}]:unsafe_path", {"path": ch["path"]})

            p_norm = _norm_path_for_compare(ch["path"])
            if p_norm not in allowed_paths:
                raise GateError(1, "FAIL_ALLOWLIST_VIOLATION", f"patch_change[{i}]:path_not_allowed", {"path": ch["path"]})

            total_chars = _sum_and_check_str(ch["path"], f"patch_change[{i}].path", c, total_chars)

            if ch["op"] == "write":
                if "content" not in ch or not isinstance(ch["content"], str):
                    raise GateError(1, "FAIL_CONTRACT_MISSING_FIELD", f"patch_change[{i}]:content_required")
                total_chars = _sum_and_check_str(ch["content"], f"patch_change[{i}].content", c, total_chars)

    if total_chars > c.max_total_chars:
        raise GateError(1, "FAIL_OVERSIZED_TEXT", "total_chars_exceeded", {"total_chars": total_chars, "max_total_chars": c.max_total_chars})
...
def main() -> int:
    ...
    try:
        req = _read_json_file(Path(args.JobRequest), max_bytes=500_000, where="job_request_file")
        res = _read_json_file(Path(args.JobResult), max_bytes=5_000_000, where="job_result_file")

        run_id, c, allowed_paths, allowed_paths_ref, prompt_ref = _validate_job_request(req, repo_root)
