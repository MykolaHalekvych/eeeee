...
def main() -> int:
    ...
    try:
        ...
        dump({"schema":"foundry_plan_v0","ok":False,"exit_code":EXIT_INFRA,"error":str(e)})
        return EXIT_INFRA

    try:
        cp = load_control_plane(repo_root, Path(args.control_plane))
    except Exception as e:
        dump({"schema":"foundry_plan_v0","ok":False,"exit_code":EXIT_INFRA,"error":f"control_plane load failed: {type(e).__name__}: {e}"})
        return EXIT_INFRA
    ...
    steps = [
        {"step_id":"S0_POLICY_SUMMARY", "action":"policy_summary", "required_permission": None},
        {"step_id":"S1_GATE",          "action":"gate",          "required_permission": None},
        {"step_id":"S2_BUILD_BUNDLE",  "action":"build_bundle",  "required_permission": "ALLOW_BUILD"},
        {"step_id":"S3_HASHES",        "action":"write_hashes",  "required_permission": "ALLOW_BUILD"},
        {"step_id":"S4_EVIDENCE",      "action":"write_evidence","required_permission": "ALLOW_BUILD"},
        {"step_id":"S5_RUNBOOK",       "action":"write_runbook", "required_permission": "ALLOW_BUILD"},
        {"step_id":"S6_RELEASE",       "action":"release",       "required_permission": "ALLOW_EXPORT"}
    ]
