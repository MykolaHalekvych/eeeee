...
def main() -> int:
    ...
    print(f"REPLAY total={report['total']} matches={report['matches']} mismatches={report['mismatches']}")

    if report["mismatches"] > 0:
        print("Mismatch examples (first up to 5):")
        for ex in report["mismatch_examples"]:
            print(f"- line={ex['line']} event_id={ex.get('event_id')} expected={ex['expected']} actual={ex['actual']}")
