...
class TestStage5FillDedupV0(unittest.TestCase):
    ...
    def test_fill_dedup_key_stable(self):
        ev1 = BrokerEvent(EventType.FILL, 1018, "oid_1018", "AAPL", 1.0, 0.0, "execId:X1", "2026-01-02T08:00:00Z")
        ev2 = BrokerEvent(EventType.FILL, 1018, "oid_1018", "AAPL", 1.00, 0.0, "execId:X1", "2026-01-02T08:00:00Z")
        self.assertEqual(_fill_dedup_key_v0(ev1), _fill_dedup_key_v0(ev2))

    def test_fill_dedup_key_diff_execid(self):
        ev1 = BrokerEvent(EventType.FILL, 1018, "oid_1018", "AAPL", 1.0, 0.0, "execId:X1", "2026-01-02T08:00:00Z")
        ev2 = BrokerEvent(EventType.FILL, 1018, "oid_1018", "AAPL", 1.0, 0.0, "execId:X2", "2026-01-02T08:00:00Z")
