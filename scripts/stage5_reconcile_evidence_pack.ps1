...
function _module_not_found([string]$stderrPath) {
  ...
  return ($t -match "ModuleNotFoundError" -or $t -match "No module named")
}
...
function _try_open_orders([string]$exe, [object[]]$prefix, [string]$IbHost, [int]$port, [int]$clientId, [int]$cto, [int]$to, [int]$wait, [string]$outJsonl, [string]$stdoutPath, [string]$stderrPath) {
  ...
  foreach ($m in $mods) {
    ...
    if ($rc -eq 0 -and (Test-Path -LiteralPath $outJsonl)) { return [ordered]@{ ok=$true; module=$m; exit_code=0 } }

    if (-not (_module_not_found $stderrPath)) {
...
try {
  ...
  $ordTry = _try_open_orders $pyExe $prefix $IbHost $Port $ClientId $ConnectTimeoutSec $TimeoutSec $WaitSec $ordJsonl $ordStd $ordErr
  ...
  if (-not $posOk)  { $issues += "positions_ok=false" }
    if ($posErrMsg) {
    $m = [string]$posErrMsg
    $codes = @([regex]::Matches($m,'code=(\d+)') | ForEach-Object { [CmdletBinding()]
param(
  [string]$Repo = "",
  [string]$IbHost = "",
  [int]$Port = 0,
...
try {
  ...
  ($summary | ConvertTo-Json -Depth 14) | Set-Content -LiteralPath (Join-Path $runDir "reconcile_summary.json") -Encoding UTF8
  ($summary | ConvertTo-Json -Depth 14) | Set-Content -LiteralPath $latest -Encoding UTF8
...
.Groups[1].Value } | Sort-Object -Unique)
    $allowed = @('2104','2106','2158')
    $other = @($codes | Where-Object { $allowed -notcontains [CmdletBinding()]
param(
  [string]$Repo = "",
  [string]$IbHost = "",
  [int]$Port = 0,
...
try {
  ...
  exit 2
    ...
    # WARN only if there are codes outside the farm-ok set (or no codes at all)
    if (($codes.Count -eq 0) -or ($other.Count -gt 0)) {
      $warns += ("positions_error=" + $posErrMsg)
    } else {
      # INFO: farm connection OK
    }
  }
if (-not $ordOk)  { $issues += ("open_orders_ok=false module=" + [string]$ordTry.module + " exit=" + [string]$ordTry.exit_code) }
