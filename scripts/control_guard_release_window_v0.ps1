...
if ([string]::IsNullOrWhiteSpace($ControlJobType)) { $ControlJobType = "FOUNDRY_RELEASE_WINDOW_V1" }
if ($ControlJobType -ne "FOUNDRY_RELEASE_WINDOW_V1") {
  EmitAndExit 1 "CONTROL_TOKEN.JOB_TYPE_UNEXPECTED" ("expected FOUNDRY_RELEASE_WINDOW_V1, got: " + $ControlJobType)
}
...
try {
  ...
  else { EmitAndExit 2 "INNER_ARGS_JSON.INVALID" "Expected JSON array of strings" }
} catch {
  EmitAndExit 2 "INNER_ARGS_JSON.BAD_JSON" $_.Exception.Message
}

# pass token params into inner too
