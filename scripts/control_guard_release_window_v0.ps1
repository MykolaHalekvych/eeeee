param(
  ...
  [string]$ControlRunId = "",
  [string]$ControlJobType = "FOUNDRY_RELEASE_WINDOW_V1",
  [string]$InnerScript = "",
...
# default inner script early (so placeholder includes it)
if ([string]::IsNullOrWhiteSpace($InnerScript)) {
  $InnerScript = Join-Path $PSScriptRoot "factory_release_window_v1.ps1"
}
...
$expectedJobType = "FOUNDRY_RELEASE_WINDOW_V1"
...
try {
  ...
  elseif ($parsed -is [System.Array]) { foreach ($x in $parsed) { $innerArgs += [string]$x } }
  else { _Deny "INNER_ARGS_JSON.INVALID" "Expected JSON array of strings" 2 }
} catch { _Deny "INNER_ARGS_JSON.BAD_JSON" $_.Exception.Message 2 }

for ($i=0; $i -lt $innerArgs.Count; $i++) {
  if ([string]::IsNullOrWhiteSpace($innerArgs[$i])) { _Deny "INNER_ARGS.INVALID_EMPTY" ("args contains empty element at index=" + $i) 2 }
}
...
function _InvokeInnerProcess([string[]]$argsToUse, [string]$attemptTag) {
  ...
  for ($i=0; $i -lt $psArgs.Count; $i++) {
    if ([string]::IsNullOrWhiteSpace($psArgs[$i])) {
      return @{ ok=$false; rc=2; stdout_path=$stdoutPath; stderr_path=$stderrPath; err=("psArgs has null/empty at index=" + $i) }
    }
  }
  ...
  try {
    ...
    return @{ ok=$false; rc=2; stdout_path=$stdoutPath; stderr_path=$stderrPath; err=("Start-Process failed: " + $_.Exception.Message) }
  }

  if (-not $proc -or -not $proc.Id) {
    return @{ ok=$false; rc=2; stdout_path=$stdoutPath; stderr_path=$stderrPath; err="Start-Process returned null proc or missing Id" }
  }
  ...
  return @{ ok=$true; rc=$rc; stdout_path=$stdoutPath; stderr_path=$stderrPath; err="" }
}
...
$args1 = $innerArgs
if ($PassTokenArgs -eq "YES") { $args1 = $tokenArgs + $innerArgs; $token_args_used = $true }

$r1 = _InvokeInnerProcess $args1 "A1"
if (-not $r1.ok) { _Deny "INNER.INVOKE.FAIL" $r1.err 2 }
...
if ($token_args_used -and $RetryWithoutTokenArgs -eq "YES") {
  $all = ((_ReadTail $stdoutPath 200) -join "`n") + "`n" + ((_ReadTail $stderrPath 200) -join "`n")
  if ( ($all -match "ControlTokenPath") -and ( ($all -match "matches parameter name") -or ($all -match "cannot be found") -or ($all -match "Не удается найти") -or ($all -match "не удается найти") ) ) {
    ...
    if (-not $r2.ok) { _Deny "INNER.INVOKE.RETRY_FAIL" $r2.err 2 }
    $innerRc = [int]$r2.rc
...
$innerJson = _ExtractLastJsonObject (_ReadTail $stdoutPath 400)
if ($null -eq $innerJson) { $innerJson = _ExtractLastJsonObject (_ReadTail $stderrPath 400) }

$innerReason = _PickReasonFromInnerJson $innerJson
$innerExit = $innerRc
if ($null -ne $innerJson) {
  try { if ($innerJson.PSObject.Properties.Name -contains "exit_code") { $innerExit = [int]$innerJson.exit_code } } catch { }
}

$code = 2
if ($innerExit -eq 0) { $code = 0 } elseif ($innerExit -eq 1) { $code = 1 } else { $code = 2 }

$reason = $innerReason
if ([string]::IsNullOrWhiteSpace($reason)) {
  if ($code -eq 0) { $reason = "INNER.PASS" } elseif ($code -eq 1) { $reason = "INNER.FAIL" } else { $reason = "INNER.INFRA" }
}
