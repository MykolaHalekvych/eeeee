    ...
    if (Test-Path -LiteralPath $proofCursor) {
      ...
      if ($lastDir) {
        ...
        if (Test-Path -LiteralPath $sumPath) {
          ...
          $kinds = @()
          try { $kinds = @($sum.proofs | ForEach-Object { [CmdletBinding()]
param(
  [string]$Repo = "",
  [int]$MaxTapAgeSec = 180,
  [int]$MaxEvidenceAgeSec = 900
...
.kind }) } catch { $kinds = @() }
          if ($kinds -contains "REJECTED") { $issues += "terminal_reject_detected" }
        } else {
    ...
    $warns += ("terminal_proof_check_error=" + [CmdletBinding()]
param(
  [string]$Repo = "",
  [int]$MaxTapAgeSec = 180,
  [int]$MaxEvidenceAgeSec = 900
