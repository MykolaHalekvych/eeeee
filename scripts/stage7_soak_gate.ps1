...
try {
  ...
  # --- Reconcile evidence (WARN/FAIL) ---
  $reconLatest = Join-Path $Repo "args\data\reconcile_evidence_latest.json"
  if (-not (Test-Path -LiteralPath $reconLatest)) {
    $warns += "reconcile_latest_missing"
  } else {
    $fiR = Get-Item -LiteralPath $reconLatest
    $ageR = ([DateTime]::UtcNow - $fiR.LastWriteTimeUtc).TotalSeconds
    if ($ageR -gt 600) { $warns += ("reconcile_latest_stale_sec=" + [Math]::Round($ageR,1)) }

    try {
      $r = Get-Content -LiteralPath $reconLatest -Raw | ConvertFrom-Json
      if ($r.exit_code -eq 2) { $issues += "reconcile_exit_code=2" }
      elseif ($r.exit_code -eq 1) { $warns += "reconcile_exit_code=1" }
    } catch {
      $warns += ("reconcile_parse_error=" + [CmdletBinding()]
param(
  [string]$Repo = "",
  [int]$MaxTapAgeSec = 180,
  [int]$MaxEvidenceAgeSec = 900,
