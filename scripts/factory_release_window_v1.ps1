...
if (-not (Test-Path -LiteralPath ".\.args_engine_repo")) {
  EmitJsonAndExit @{ schema="factory_release_window_v1"; ok=$false; ts_utc=$ts; error="ENG guard failed: missing .args_engine_repo" } 2
}

$stopFlag = ".\args\control\stop.flag"
if (Test-Path -LiteralPath $stopFlag) {
  EmitJsonAndExit @{ schema="factory_release_window_v1"; ok=$false; ts_utc=$ts; error="HALT: stop.flag present"; stop_flag=$stopFlag } 2
}
...
try {
  ...
  EmitJsonAndExit @{ schema="factory_release_window_v1"; ok=$false; ts_utc=$ts; repo=$repoPath; control_plane=$ControlPlane; error=("control_plane backup failed: " + $_.Exception.Message) } 2
}
...
try {
  # Enable export for this window
  Set-AllowExport $true

  # 0) Gate (optional but standard)
  ...
  if ([string]::IsNullOrWhiteSpace($rid)) {
    $releaseCmd = "$Python -m args.foundry.release_v0 --control-plane .\$ControlPlane --product $Product --factory $Factory"
  } else {
    $releaseCmd = "$Python -m args.foundry.release_v0 --control-plane .\$ControlPlane --product $Product --factory $Factory --release-id $rid"
  }
  $releaseJson = Invoke-Expression $releaseCmd
  $releaseRc = $LASTEXITCODE
  if ($releaseRc -ne 0) { throw "release failed rc=$releaseRc" }
  ...
  $code = 2
  if ($gateRc -eq 1 -or $planRc -eq 1 -or $releaseRc -eq 1) { $code = 1 }
