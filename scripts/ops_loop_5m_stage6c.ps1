...
# Stage 6C wrapper: refresh open-orders snapshot, then run inner loop.
...
param(
  ...
  # DO NOT use $Host: it's a read-only automatic variable in PowerShell
...
function New-RunId { return ("{0:yyyyMMdd_HHmmssfff}_pid{1}" -f (Get-Date), $PID) }
...
function Acquire-ExclusiveLock([string]$LockPath, [string]$runId) {
  ...
  try {
    ...
    $meta = @{ run_id=$runId; pid=$PID; start_utc=(Get-Date).ToUniversalTime().ToString("o") } | ConvertTo-Json -Compress
    $fs.SetLength(0)
...
# --- logging ---
$logFull = if ([System.IO.Path]::IsPathRooted($MainLogFile)) { $MainLogFile } else { Join-Path $Repo $MainLogFile }
...
try {
  $lockFull = if ([System.IO.Path]::IsPathRooted($WrapperLockFile)) { $WrapperLockFile } else { Join-Path $Repo $WrapperLockFile }
  $lockHandle = Acquire-ExclusiveLock $lockFull $RunId
  if (-not $lockHandle) {
    # another wrapper instance is running => success skip
    $exitCode = 0
    return
  }
  ...
  $snapshotOutFull = if ([System.IO.Path]::IsPathRooted($SnapshotOut)) { $SnapshotOut } else { Join-Path $Repo $SnapshotOut }
  ...
  for ($attempt=1; $attempt -le $SnapshotMaxAttempts; $attempt++) {
    ...
    try {
      $pyArgs = @()
      if ($PythonVerArg) { $pyArgs += $PythonVerArg }
      $pyArgs += @(
  ...
  if ($snapExit -ne 0) { Log "WARNING: snapshot failed (exit_code=$snapExit). Continuing." }

  # --- inner loop ---
  $innerFull = if ([System.IO.Path]::IsPathRooted($InnerLoop)) { $InnerLoop } else { Join-Path $Repo $InnerLoop }
