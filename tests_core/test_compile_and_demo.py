...
class TestStage5Basic(unittest.TestCase):
    def test_fill_goes_done_and_flat(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            eng = Engine(repo_root=repo, run_id="r1", control_plane_path=cp_path, broker=broker)
    ...
    def test_stop_flag_blocks_place(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            cp = ControlPlane(global_mode="ONLY_EXITS", run_root="runs", kill_switch_file="stop.flag", cool_down_seconds=1)
            cp_path = repo / "control_plane.json"
            cp.save(cp_path)

            broker = FakeBroker(scenario="fill", positions={"AAPL": 2.0})
            eng = Engine(repo_root=repo, run_id="r2", control_plane_path=cp_path, broker=broker)
